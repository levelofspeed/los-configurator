...
if logo:
    # Center the logo with narrower width and smaller size
    _, c, _ = st.columns([1, 2, 1])  # central column ratio
    c.image(logo, width=100)
...
""")
    ...
    try:
        ...
        else: server=smtplib.SMTP(host,port);server.ehlo();server.starttls();server.ehlo()
