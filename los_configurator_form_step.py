...
translations = {  # (same dictionary as before, trimmed here for brevity)
...
# ---------------- Header ----------------------
_, col_lang = st.columns([10,2])  # language selector right
with col_lang:
...
""")
    try:
        ...
        if uploaded_file is not None:
            requests.post(f"https://api.telegram.org/bot{TOKEN}/sendDocument", data={"chat_id": CHAT}, files={"document": (uploaded_file.name, uploaded_file.read(), uploaded_file.type or "application/octet-stream")}, timeout=20)
            uploaded_file.seek(0)  # reset for email PDF
    except Exception as e:
