...
class SafeTranslations(UserDict):  # возвращает ключ, если перевода нет
...
@st.cache_data
def load_db():
    with open(os.path.join(os.getcwd(), "data", "full_database.json"), encoding="utf-8") as f:
        return json.load(f)
def _prune(n): return {k:_prune(v) for k,v in n.items() if v not in ("",None,{},[])} if isinstance(n,dict) else n
database = _prune(load_db())

def _clear(*keys): [st.session_state.pop(k,None) for k in keys]
...
if not email or "@" not in email: st.error(_t["error_email"]); st.stop()
